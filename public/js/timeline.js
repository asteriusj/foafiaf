...
function buildDataSet(graph, cb) {
	...
	//
    var spokeNodes = getNodes(graph, 'foafiaf:Spoke')        
	//console.log('spokeNodes', spokeNodes)
	
	const list = []
	for (var s=0; s<spokeNodes.length; s++) {
    ...
    for (var i=0; i<list.length; i++) {
        var _parentid = "foafiaf:" + list[i] ;
        var _id = list[i] ;
        if (_id) _id = _id.replace('&', '') ;
        var _name = list[i].replace('Spoke_',' ') ;
        ...
        // prep dataset
        var measureNodes = getNodesByParent(graph, 'foafiaf:Measure', 'foafiaf:Spoke', _parentid)
        var dataset = prepNodes(measureNodes);
    ...
    var tl = setGroupTimeline(myGroups, myItems)
    
// function onMouseover (event) {
//   var properties = tl.getEventProperties(event);
...
function setGroupTimeline(groups, items){
    ...
    var container_ = document.getElementById('visualization'); 
    console.log(groups, items, container_)
    
    var timeline_ = new vis.Timeline(container_, items, groups, options);
    console.log('timeline_', timeline_)
    

function onMouseover (event) {
  var properties = timeline_.getEventProperties(event);
  // properties contains things like node id, group, x, y, time, etc.
  console.log('mouseover properties:', properties);
  
  // mouse over has occurred
  
  console.log('properties.item',properties.item)
  var id_ = properties.item || null;
  console.log('id_', id_)
  var group_ = properties.group || null;
  console.log('group_', group_)
  
  if (group_ != null) {
    var header = document.getElementsByClassName('propsheet-header');
    header[0].innerHTML = group_ ;
  }
  ...
  if (id_ != null) {
    ...
    footer[0].innerHTML = id_ ;
  }
  
}
container_.addEventListener('mouseover', onMouseover)
...
function prepNodes(measureNodes) {
	...
	for (var i = 0; i < measureNodes.length; i++) {
	    var item = {}; //console.log('measureNodes[i]', measureNodes[i])
		var _id = measureNodes[i]['@id'];
		var _segment = measureNodes[i]["foafiaf:Segment"];
		var _spoke = measureNodes[i]["foafiaf:Spoke"] || '';
		
		if (_spoke.length >1)  _spoke = _spoke[1] ;
        if (typeof _spoke == 'string' || _spoke instanceof String) _spoke = _spoke.replace('&', '') ;
		
		var _strategy = measureNodes[i]["foafiaf:Strategy"];
		var _project = measureNodes[i]["foafiaf:Project"];
		var _start = measureNodes[i]["foafiaf:startdate"] || null;
		var _end = measureNodes[i]["foafiaf:enddate"] || null;
		var _status = measureNodes[i]["foafiaf:status"] || "";
		var _color = measureNodes[i]["foafiaf:color"] || "";
		var _content = measureNodes[i]["foafiaf:shortname"] || measureNodes[i]["rdfs:label"];
		var _category = measureNodes[i]["foafiaf:categories"] || null;
		
		item.id = _id;
		//item.group = _spoke.substring(_spoke.indexOf(":") + 1);
		var _group = measureNodes[i]["foafiaf:Spoke"] || '';
		if (_group) _group = _group.replace('foafiaf:', '') ;
	    if (_group) _group = _group.replace('&', '') ;
	    item.group = _group;
	    
	   // var _subgroup = null          use sungroup for sequence of milestones?
